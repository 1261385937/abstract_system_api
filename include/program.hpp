...
public:
    auto get_executable_path() { return api::get_executable_path(); }

    auto get_executable_parent_path() { return api::get_executable_path<1>(); }

    auto get_cpu_occupy(std::error_code& ec) {
        return api::get_self_cpu_occupy(ec);
    }

    auto calculate_cpu_usage(const self_cpu_occupy& pre,
    ...
    auto is_in_container() { return api::is_in_container(); }
};
