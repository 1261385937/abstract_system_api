...
inline std::string get_os_info(std::error_code& ec) {
    ...
    if (os.dwMinorVersion == 1) {
        os_info = os.wProductType == VER_NT_WORKSTATION
                      ? "windows 7"
...
inline network_card_t get_network_card(std::error_code& ec) {
    ...
    auto adapter_buf =
        std::shared_ptr<char>(new char[buf_len], [](char* p) { delete[] p; });
    ...
    for (; currnet != nullptr; currnet = currnet->Next) {
        ...
        card.is_down =
            (currnet->OperStatus == IF_OPER_STATUS::IfOperStatusDown);
...
inline card_flow get_network_card_flow(C&& c, std::error_code& ec) {
    ...
    for (DWORD i = 0; i != if_table->dwNumEntries; ++i) {
        ...
        flow.emplace(std::move(real_name),
                     std::pair{if_table->table[i].dwInOctets,
                               if_table->table[i].dwOutOctets});
    }
    return flow;
}
...
inline disk_info get_disk_info(std::string_view name, std::error_code& ec) {
    ...
    disk_info info{};
    auto ret =
        GetDiskFreeSpaceExA(name.data(), (PULARGE_INTEGER)&info.available_size,
                            (PULARGE_INTEGER)&info.total_size, nullptr);
    if (ret == 0) {
        ec = std::error_code(GetLastError(), std::system_category());
        return {};
    }
